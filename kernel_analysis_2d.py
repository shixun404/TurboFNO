...
def analyze_performance(target_variant_name):
    ...
    # Read CSV
    try:
        df = pd.read_csv('nsys_kernel_analysis_2d.csv')
    except FileNotFoundError:
        print("Error: 'nsys_kernel_analysis_2d.csv' not found. Please ensure the file is in the correct directory.")
        return
    ...
    if not winning_params:
        ...
        for param_combo in winning_params:
            ...
            baseline_variant_name = next((v for v in all_variants if v.endswith('_E')), None)
            
            if not baseline_variant_name:
                print("  Could not determine baseline variant (ending in '_E'). Showing absolute times:")
                for variant, total_time in sorted(variant_times.items()):
                    print(f"    {variant}: {total_time:.4f} ms")
                continue

            baseline_time = variant_times.get(baseline_variant_name)

            if baseline_time is None or baseline_time == 0:
                print(f"  Baseline '{baseline_variant_name}' time not available or is zero. Showing absolute times:")
                for variant, total_time in sorted(variant_times.items()):
                    print(f"    {variant}: {total_time:.4f} ms")
            else:
                print(f"  Performance relative to baseline '{baseline_variant_name}':")
                for variant, total_time in sorted(variant_times.items()):
                    percentage = (total_time / baseline_time) * 100
                    print(f"    {variant}: {total_time:.4f} ms ({percentage:.2f}%)")

def main():
    target_variant_name = input("请输入您想分析的variant name (例如: TurboFNO_2D_D): ")
