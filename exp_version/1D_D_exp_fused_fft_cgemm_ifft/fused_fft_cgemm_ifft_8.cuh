...
__global__ void fused_fft_cgemm_ifft_8(int M, int N, int K, float2 *FFT_input, float2 *B, float2 *C, float2 *FFT_output, float2 alpha, float2 beta){
    ...
    for(int i = 0; i < LOAD_PER_THREAD_B; i++){
        ...
        fft_8_fused(gFFT_input + BID_X * 256, shared_mem_float2, sFFT, M * 4);
    ...
    for(int k = 0; k < K - THREADBLOCK_K; k += THREADBLOCK_K){
        ...
        // if(threadIdx.x < THREADBLOCK_K * 128 / 8){
            fft_8_fused(gFFT_input + BID_X * 256 + (k + THREADBLOCK_K) * M * 4, shared_mem_float2, sFFT, M * 4); 
        // }
    ...
    for(int tid_start = 0; tid_start < THREADBLOCK_N; tid_start += THREADBLOCK_K){
        ...
        __syncthreads();
        fft_8_fused_output(sFFT, gFFT_output + BID_X * 256 + (BID_Y * THREADBLOCK_N + tid_start) * M * 4, sFFT, M * 4);
    }
