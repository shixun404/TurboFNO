...
#include "fft_radix_2_logN_8_upload_0_fused_trunc.cuh"
#include "fft_radix_2_logN_8_upload_0_fused_output_trunc.cuh"
...
__global__ void fused_fft_cgemm_ifft_8(int M, int N, int K, float2 *FFT_input, float2 *B, float2 *C, float2 *FFT_output, float2 alpha, float2 beta){
    ...
    fft_8_fused_trunc(gFFT_input + BID_X * 256, shared_mem_float2, sFFT, M * 4);
    ...
    for(int k = 0; k < K - THREADBLOCK_K; k += THREADBLOCK_K){
        ...
        // if(threadIdx.x < THREADBLOCK_K * 128 / 8){
            fft_8_fused_trunc(gFFT_input + BID_X * 256 + (k + THREADBLOCK_K) * M * 4, shared_mem_float2, sFFT, M * 4); 
        // }
        ...
        if(tid_start <= threadblock_C_col && threadblock_C_col < tid_start + THREADBLOCK_K) {
            ...
            for(int j = 0; j < THREAD_N; j++){
                ...
                for(int i = 0; i < THREAD_M; i++){
                    ...
                    + (threadblock_C_col + j - tid_start) * 256] = c[(i + (TID / 4)) % THREAD_M][j];
                }
            }
        }
        __syncthreads();
        fft_8_fused_output_trunc(sFFT, gFFT_output + BID_X * 256 + (BID_Y * THREADBLOCK_N + tid_start) * M * 4, sFFT, M * 4);
    }
