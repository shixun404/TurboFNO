...
def main():
    # Read CSV
    df = pd.read_csv('nsys_kernel_analysis.csv')
    ...
    all_variants = ['TurboFNO_2D_A', 'TurboFNO_2D_B', 'TurboFNO_2D_C', 'TurboFNO_2D_D', 'TurboFNO_2D_E']
    all_kernels = df['detailed_kernel_name'].unique()  # Use detailed kernel names
    
    # Create output directory
    os.makedirs('kernel_breakdown_plots', exist_ok=True)
    ...
    for i, param_combo in enumerate(param_combos):
        ...
        filepath = os.path.join('kernel_breakdown_plots', filename)
        ...
        ax.set_xticklabels([v.replace('TurboFNO_2D_', '') for v in all_variants])
    ...
    print(f"\nDone! Generated {len(param_combos)} plots in 'kernel_breakdown_plots/' directory")
