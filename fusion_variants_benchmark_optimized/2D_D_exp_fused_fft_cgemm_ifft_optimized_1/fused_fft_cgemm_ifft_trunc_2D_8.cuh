...
__global__ void fused_fft_cgemm_ifft_8(int M, int N, int K, float2 *FFT_input, float2 *B, float2 *C, float2 *FFT_output, float2 alpha, float2 beta){
    ...
    #pragma unroll
    for(int tid_start = 0; tid_start < THREADBLOCK_N; tid_start += THREADBLOCK_K){
        // int tid_start = ts * THREADBLOCK_K;
        __syncthreads();
        if(tid_start <= threadblock_C_col && threadblock_C_col < tid_start + THREADBLOCK_K) {
            #pragma unroll
            for(int j = 0; j < THREAD_N; j++){
        ...
        __syncthreads();
        fft_8_fused_output(sFFT, gFFT_output + BID_Y * 256 + (BID_X * THREADBLOCK_N + tid_start) * M * 4, sFFT, M * 4);
    }
