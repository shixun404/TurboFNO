...
__global__ __launch_bounds__(THREAD_NUM) void fused_fft_cgemm_7(int M, int N, int K, float2 *FFT_input, float2 *B, float2 *C, float2 alpha, float2 beta){
    ...
    float2* sFFT = shared_mem_float2 + THREADBLOCK_M * THREADBLOCK_K + (THREADBLOCK_N + 2) * THREADBLOCK_K;
