...
__global__ void fused_fft_cgemm_ifft_9(int M, int N, int K, float2 *FFT_input, float2 *B, float2 *C, float2 *FFT_output, float2 alpha, float2 beta){
        ...
        if(tid_start <= threadblock_C_col && threadblock_C_col < tid_start + THREADBLOCK_K) {
            ...
            for(int j = 0; j < THREAD_N; j++){
                ...
                for(int i = 0; i < THREAD_M; i++){
                    ...
                    + (threadblock_C_col + j - tid_start) * 512] = c[(i + (TID / 4)) % THREAD_M][j];
                }
            }
        }
        __syncthreads();
