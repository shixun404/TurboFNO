...
Simple version without pandas dependency - 1D version
"""
import os
import re
import csv
...
def parse_filename(filename):
    ...
    # Example: TurboFNO_1D_A_bs1_dx128_dy128_n64_k8.txt
    parts = filename.replace('.txt', '').split('_')
    
    variant = None
    params = {}
    
    # Find variant name (TurboFNO_1D_X)
...
def main():
    parser = argparse.ArgumentParser(description='Analyze nsys stats output files for 1D variants')
    parser.add_argument('--stats-dir', '-s', 
                       default='nsys_stats_1d',
                       help='Directory containing nsys stats files')
    parser.add_argument('--output-csv', '-o',
                       default='nsys_kernel_analysis_1d.csv',
                       help='Output CSV file')
    parser.add_argument('--summary-dir', 
                       default='analysis_reports_1d',
                       help='Directory for summary reports')
    
    args = parser.parse_args()
    
    print("NSYS Results Analysis Tool (Simple Version - 1D)")
    print("=" * 50)
